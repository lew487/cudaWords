...
#define iterationsPerBlock (n*n/(max_number_of_threads*4)) +1
...
__device__ int cmpHash(unsigned int &hash1, unsigned int &hash2)
{
	//printf("%c %c", c1, c2);
	return hash1==hash2;
...
__device__ unsigned int getWordHash(char* str, uint len) {
...
__global__ void tkernel(struct gpuStringArray *a, gpuStringArray *b, int * result, int *threadsNumber)
{
	...
	for(int i=0; i<iterationsPerThread; i++)
	{
		...
		{
			result[idx]=cmpHash(a->hash[j], b->hash[idx]);
...
__global__ void hashWords(struct gpuStringArray *a, gpuStringArray *b, int *threadsNumber)
{
	int idx = threadIdx.x + blockIdx.x * blockDim.x;
	
	int iterations=a->size;
	int iterationsPerThread=iterations/(*threadsNumber)+1;
	
	for(int i=0; i<iterationsPerThread; i++)
	{
		if(idx>iterations) break;
		char * s = a->data + a->pos[idx];
		a->hash[idx]=getWordHash(s, a->length[idx]) ;
	...
	{
		if(idx>iterations) break;
		char * s = b->data + b->pos[idx];
		b->hash[idx]=getWordHash(s, b->length[idx]) ;
...
gpuStringArray* allocateGpuStringArray(vector<unsigned int> positions, vector<unsigned int> lenghts, string str)
{
	...
	size_t hashSize=sizeof(unsigned int)*(lenghts.size()+1);
	cudaMalloc((void **) &cudaHash, hashSize);	
	//cudaMemcpy(cudaLength, inputLengths, lengthSize, cudaMemcpyHostToDevice);
...
int main(int argc, char** argv)
{
	...
	hashWords<<< grid_size, block_size>>> (inputCuda, tempCuda, cudaThreadsNumber);
